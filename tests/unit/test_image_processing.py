...
class TestCreateReferenceMask:
    def test_output_is_binary(self, synthetic_seg_3d):
        ...
        mask = create_reference_mask(synthetic_seg_3d, resample_factor=2.0, dilation_radius=2)
    ...
    def test_dilated_mask_volume_exceeds_binary_input(self, synthetic_seg_3d):
        ...
        mask_no_dilate = create_reference_mask(synthetic_seg_3d, resample_factor=1.0, dilation_radius=0)
        vol_no_dilate = int(
            sitk.GetArrayFromImage(mask_no_dilate.get_data()).sum()
        )

        # Build mask with dilation
        mask_dilated = create_reference_mask(synthetic_seg_3d, resample_factor=1.0, dilation_radius=3)
        vol_dilated = int(
    ...
    def test_output_spacing_matches_resample_factor(self, synthetic_seg_3d):
        """Output spacing must be approximately input_spacing × resample_factor."""
        factor = 2.0
        input_spacing = synthetic_seg_3d.get_spacing()  # (1, 1, 1)
        mask = create_reference_mask(synthetic_seg_3d, resample_factor=factor, dilation_radius=1)
        out_spacing = mask.get_spacing()
        for i in range(3):
            assert out_spacing[i] == pytest.approx(input_spacing[i] * factor, rel=0.05), (
                f"Axis {i}: expected {input_spacing[i] * factor}, got {out_spacing[i]}"
            )

    def test_returns_image_wrapper(self, synthetic_seg_3d):
        mask = create_reference_mask(synthetic_seg_3d, resample_factor=1.0, dilation_radius=1)
...
class TestCreateHighResMask:
    def test_output_dimensions_match_ref_seg(self, synthetic_seg_3d):
        ...
        low_res_mask = create_reference_mask(synthetic_seg_3d, resample_factor=2.0, dilation_radius=1)
